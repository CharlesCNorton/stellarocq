...
The checker also takes `--project` with `--spectrum` and `--band`, `--lower`, `--integrate`, `--taylor`, `--slot3` and `--mercier`, and `gen/` holds the drivers for the Mercier profile, the Boozer stream function, the convergence study, the radial scan and the resonance study. The Python tools need `numpy` and `netCDF4`, and `h5py` for a VMEC++ HDF5 output.
